...
def test_upload_and_visualize(authenticated_page: Page):
    ...
    # Open upload modal, then upload file
    page.locator("button:has-text('Upload')").first.click()
    page.set_input_files("#fileInputModal", csv_path)

    # 2. Verify upload succeeded by checking that file appears in sidebar
    page.wait_for_selector("#logItems li:not(.empty-state)", timeout=5000)
    expect(page.locator("#logItems li:not(.empty-state)")).to_contain_text("dummy_test.csv")
...
def test_ai_drawer_and_analysis_flow(authenticated_page: Page):
    ...
    page.set_input_files("#fileInputModal", csv_path)
    page.wait_for_selector("#logItems li:not(.empty-state)", timeout=5000)
    expect(page.locator("#logItems li:not(.empty-state)")).to_contain_text("ai_test.csv")

    # 2. Trigger AI Analysis
    # The FAB AI should be enabled now
    page.wait_for_timeout(500)  # Wait for UI to stabilize
...
def test_ui_interactions(authenticated_page: Page):
    ...
    page.set_input_files("#fileInputModal", csv_path)
    page.wait_for_selector("#logItems li:not(.empty-state)", timeout=5000)
    expect(page.locator("#logItems li:not(.empty-state)")).to_contain_text("filter_test.csv")
